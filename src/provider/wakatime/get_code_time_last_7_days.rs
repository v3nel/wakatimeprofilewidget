use crate::{
    AppResult, AppState,
    scripts::{format_digital_coding_time, get_last_7_days_dates},
    types::WakatimeLast7DaysCodingTimeResponse,
};

pub async fn get_code_time_last_7_days(state: &AppState) -> AppResult<String> {
...
async fn request_code_time_last_7_days(
    ...
    let response = state
        .http
        .get(format!(
            "https://wakatime.com/api/v1/users/current/summaries?start={today_minus_7}&end={today}"
        ))
